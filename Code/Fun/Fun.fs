...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArgs) -> 
      ...
      | Closure (f, x, fBody, fDeclEnv) ->
        let xVals = List.fold (fun acc elem -> Int(eval elem env) :: acc) [] eArgs
        let argValList = List.zip x xVals
        let fBodyEnv = List.fold (fun acc elem -> (elem) :: acc) ((f, fClosure) :: fDeclEnv) argValList
        //let xVal = Int(eval eArgs[0] env)
        //let fBodyEnv = (x[0], xVal) :: (f, fClosure) :: fDeclEnv
        eval fBody fBodyEnv
